...
export default function Guides() {
 ...
 useEffect(() => {
    ...
    setListOfRetreats(retreats1);
          ...
          const q3 = query(collection(db, "guides"), where("price", "<=", selectedPrice));
      ...
      setListOfRetreats([])
      const retreats2=[]
          const q3 = await query(collection(db, "guides"), where("type1", "==", selectedType), where("isDisplayed", "==", true), where("location","==",selectedLocation));
        getDocs(q3).then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
          console.log(doc.id, " => ", doc.data().name);
          retreats2.push({ ...doc.data() }); 
      ...
      setListOfRetreats([])
      const retreats2=[]
          const q3 = await query(collection(db, "guides"), where("type1", "==", selectedType), where("location","==",selectedLocation), where("price","<=",selectedPrice));
        getDocs(q3).then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
          console.log(doc.id, " => ", doc.data().name);
          retreats2.push({ ...doc.data() }); 
  ...
  return (
    ...
    <div className="justify-center items-center grid h-screen absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${pic})` }}>
      ...
      <div className="w-full max-w-xs items-center justify-center">
        ...
        <h1 className='font-bold text-lime-700'>Sign in as a Guide</h1>
        <br/>
          <div className="mb-4">
            <label className="block text-lime-700 text-sm font-bold mb-2" htmlFor="email"> 
              Email
            </label>
            <input className="shadow appearance-none border rounded w-full py-2 px-3 text-lime-700 leading-tight focus:outline-none focus:shadow-outline" id="email" type="email" placeholder="Email" onChange={(e)=>setEmail(e.target.value)}/>
          </div>
          <div className="mb-6">
            <label className="block text-lime-700 text-sm font-bold mb-2" htmlFor="password">
              Password
            </label>
            ...
            <p className="text-lime-700 text-xs italic">Please choose a password.</p>
          </div>  
          ...
          <br/>
           <div className="grid items-center justify-between">
             <p className="inline-block align-baseline font-bold text-sm text-lime-700">
  ...
  <div className='container'>
     <div className="max-w-3xl mx-auto text-center">
          <h2 className="text-2xl md:text-3xl font-serif font-semibold mb-4 text-center">
            Search for the perfect Guide!
          </h2>
   <div className="flex flex-col lg:flex-row sm:flex-row gap-3 max-w-xl mx-auto">
